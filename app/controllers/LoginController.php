	...
	class LoginController {
		...
		private function loginUpdate(): void{  
			if(isset($_SESSION['user']) && isset($_SESSION['user']['id'])){
				...
				$query = "INSERT INTO visites (administrateurs_id, login_date) VALUES (".$id.",'".$login_date."')";
				$stmt = $this->pdo->prepare($query);
				$stmt->execute();
			}
		}
			...
			if(isset($_SESSION['user']) && isset($_SESSION['user']['id']) && isset($_SESSION['user']['login_date'])){
				try {
					...
					if($this->pdo){
						...
						$query = "UPDATE visites SET logout_date = :logout_date WHERE administrateurs_id = :id AND login_date = :login_date";
						$stmt = $this->pdo->prepare($query);
		...
		private function sessionKeyUpdate(): void{  
			if(isset($_SESSION['user']) && isset($_SESSION['user']['id']) && isset($_SESSION['user']['id'])){
				...
				$query = "UPDATE administrateurs SET sessionkey = :sessionkey WHERE id = :id";
				$stmt = $this->pdo->prepare($query);
